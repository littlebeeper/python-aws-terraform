...
resource "aws_iam_policy" "secrets_manager_access" {
  ...
  description = "Allows access to secrets manager for ${var.name}"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "invoke_lambda_access" {
    name = "${var.name}-invoke-lambda-access"
    description = "Allows access to invoke lambda for ${var.name}"
    policy = jsonencode({
        Version = "2012-10-17"
        Statement = [
        {
...
resource "aws_iam_policy" "aws_exec_access" {
  name = "${var.name}-aws-exec-access"
  description = "Allows access to AWS exec for tasks in ${var.name}"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
